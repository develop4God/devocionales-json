...
from shared_validation.checks.family_check import run_family_validation_all
from shared_validation.checks.greek_hebrew_gloss import (
...
from shared_validation.checks.lexicon_family_check import (
...
from shared_validation.checks.text_checks import (
...
def validate_structure(
    ...
    if data.get("language") != lang:
        report.E(
            f"{filename}: Language mismatch - expected '{lang}', got '{data.get('language')}'"
        )
        is_valid = False
    ...
    if expected_versions:
        if isinstance(expected_versions, list):
            if data.get("version") not in expected_versions:
                report.E(
                    f"{filename}: Bible version mismatch - expected one of {expected_versions}, got '{data.get('version')}'"
                )
                is_valid = False
        else:
            if data.get("version") != expected_versions:
                report.E(
                    f"{filename}: Bible version mismatch - expected '{expected_versions}', got '{data.get('version')}'"
                )
                is_valid = False
...
def _check_key_parity(
    en_obj, trans_obj, path: str, filename: str, lang: str, report: Report
):
    ...
    if isinstance(en_obj, dict):
        ...
        if not isinstance(trans_obj, dict):
            report.E(
                f"{filename}: '{path}' is an object in EN but not in {lang.upper()}"
            )
            return
        for k in en_obj:
            ...
            if k not in trans_dict:
                report.E(
                    f"{filename}: '{child_path}' present in EN but missing in {lang.upper()}"
                )
            else:
                _check_key_parity(
                    en_obj[k], trans_dict[k], child_path, filename, lang, report
                )
        for k in trans_dict:
            if k not in en_obj:
                report.W(
                    f"{filename}: '{path}.{k}' present in {lang.upper()} but missing in EN (possible extra/stray field)"
                )
        ...
        if not isinstance(trans_obj, list):
            report.E(
                f"{filename}: '{path}' is an array in EN but not in {lang.upper()}"
            )
            return
...
def validate_content_translation(
    ...
    for i, (en_card, trans_card) in enumerate(
        zip(en_data.get("cards", []), trans_data.get("cards", []))
    ):
...
def validate_filename_format(
    filepath: Path, lang: str, report: Report, expected_languages: dict
) -> bool:
    ...
    pattern = re.compile(
        r"^[a-z0-9_]+_(" + "|".join(expected_languages.keys()) + r")_001\.json$"
    )

    if not pattern.match(filename):
        report.E(
            f"{filename}: Invalid filename format. Expected pattern: {{study_name}}_{{lang}}_001.json"
        )
        return False
    ...
    if lang_in_filename != lang:
        report.E(
            f"{filename}: Language code mismatch - file is in {lang}/ directory but filename contains '{lang_in_filename}'"
        )
        return False
...
def validate_sot_source(
    report: Report, bible_versions: dict, used_remote_sot: bool
) -> None:
...
def validate_lint(report: Report, discovery_dir: Path) -> None:
    ...
    cache = lint_json_files(
        discovery_dir, report, "discovery_scripts", severity="warning"
    )
            ...
            if not isinstance(study["files"], dict):
                ...
                if missing_langs:
                    pending_studies.append(
                        {"id": study_id, "missing_languages": sorted(missing_langs)}
                    )
                    report.W(
                        f"Study {study_id}: Missing translations for {sorted(missing_langs)}"
                    )
                ...
                for lang, filename in files.items():
                    ...
                    expected_filename = (
                        f"{study_id.replace('_001', '')}_{lang}_001.json"
                    )
                    if filename != expected_filename:
                        report.W(
                            f"Study {study_id}: File for {lang} is '{filename}', expected '{expected_filename}'"
                        )
            ...
            if not isinstance(study["titles"], dict):
                ...
                if "files" in study:
                    ...
                    if title_langs != file_langs:
                        report.W(
                            f"Study {study_id}: Title languages {sorted(title_langs)} don't match file languages {sorted(file_langs)}"
                        )
            ...
            if not isinstance(study["subtitles"], dict):
                ...
                if "files" in study:
                    ...
                    if subtitle_langs != file_langs:
                        report.W(
                            f"Study {study_id}: Subtitle languages {sorted(subtitle_langs)} don't match file languages {sorted(file_langs)}"
                        )
...
def validate_translation_files(
    ...
    for lang, expected_versions in expected_languages.items():
        ...
        if isinstance(expected_versions, list):
            report.I(
                f"Checking {lang.upper()} folder with versions: {', '.join(expected_versions)}"
            )
        else:
    ...
    for study_id, study in index_studies.items():
        ...
        for lang, filename in files.items():
            ...
            if not filepath.exists():
                report.E(
                    f"index.json: Study {study_id} lists {lang}/{filename} but file doesn't exist"
                )
    ...
    with ScriptureValidator(bible_database_dir) as validator:
        for lang, lang_studies in all_studies.items():
            ...
            for study_id, data in lang_studies.items():
                ...
                if en_data is None:
                    report.W(
                        f"{study_id} ({lang}): no EN sibling study found — skipping scripture checks"
                    )
                    continue
...
def main():
    ...
    index_data = run_report.wrap(
        "PHASE A: INDEX", validate_index_json, discovery_dir, expected_languages
    )
    ...
    if not args.scripture_only:
        ...
        all_studies = validate_translation_files(
            scratch,
            discovery_dir,
            expected_languages,
            index_studies,
