    ...
    for i, card in enumerate(cards):
            ...
            if words:
                for j, w in enumerate(words):
                    if not w.get("word") or not w.get("transliteration"):
                        report.err(
                            f"{card_ctx} {key}[{j + 1}]: missing word or transliteration"
                        )
        ...
        for sc in card.get("scripture_connections", []) or []:
            if not sc.get("reference"):
                report.err(
                    f"{card_ctx} scripture_connections: reference missing or empty"
                )
            if not nonempty(sc.get("text")):
                report.err(f"{card_ctx} scripture_connections: text missing or empty")
        qkey = (
            "discovery_questions"
            if "discovery_questions" in card
            else "reflection_questions"
            if "reflection_questions" in card
            else None
        )
        if qkey:
        ...
        for step in card.get("action_steps", []) or []:
            if not nonempty(step.get("title")) or not nonempty(step.get("description")):
                report.err(
                    f"{card_ctx} action_steps: title or description missing/empty"
                )
        prayer = card.get("prayer")
        if prayer and (
            not nonempty(prayer.get("title")) or not nonempty(prayer.get("content"))
        ):
