#!/usr/bin/env python3
"""
validate_discovery_v2.py — Parallel validator for discovery study
translations, built on the shared_validation/ library.

This is NOT a replacement for validate_discovery.py — both scripts exist
side by side so their output can be compared directly. validate_discovery.py
remains the script anyone actually runs; this one exists to prove
shared_validation/ is a safe substrate before any future decision to swap.

Tier-1 concerns (identical logic in both encounters and discovery) are
delegated to shared_validation: SOT fetch/cache, quote-anomaly checks, and
lint. Tier-3 concerns (English-leakage heuristics, Phase A index structure,
key-parity policy) stay local, copied unchanged from validate_discovery.py.
...
formats, and this script's goal is byte-identical output to the original,
not architectural purity. That reconciliation is a separate future decision.
