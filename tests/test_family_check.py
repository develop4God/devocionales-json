...
from shared_validation.family_check import Reporter, nonempty, check_drift, check_key_parity, check_filename_language_match
...
class TestCheckFilenameLanguageMatch(unittest.TestCase):
    def test_matching_filename_and_field_no_finding(self):
        report = Reporter()
        check_filename_language_match('es', Path('foo_es_001.json'), {'language': 'es'}, report)
        self.assertEqual(report.errors, 0)
        self.assertEqual(report.warnings, 0)

    def test_language_field_mismatch_flagged(self):
        report = Reporter()
        check_filename_language_match('es', Path('foo_es_001.json'), {'language': 'en'}, report)
        self.assertEqual(report.errors, 1)

    def test_filename_lang_segment_mismatch_flagged(self):
        report = Reporter()
        check_filename_language_match('es', Path('foo_en_001.json'), {'language': 'es'}, report)
        self.assertEqual(report.errors, 1)

    def test_unrecognized_filename_pattern_warns(self):
        report = Reporter()
        check_filename_language_match('es', Path('foo.json'), {'language': 'es'}, report)
...
class TestDiscoverySchemaChecks(unittest.TestCase):
    def _valid_study(self):
        return {
            'id': 'x', 'type': 'study', 'language': 'en',
            'version': 'RVR1960', 'estimated_reading_minutes': 5,
            'key_verse': {'reference': 'Juan 3:16', 'text': 'For God so loved...'},
            'cards': [{'order': 1, 'type': 'intro', 'title': 'Intro', 'subtitle': 'Sub'}],
            'tags': ['faith'], 'metadata': {'themes': ['love']},
        }
...
class TestEncountersSchemaChecks(unittest.TestCase):
    def _valid_encounter(self):
        return {
            'id': 'x', 'type': 'encounter', 'schema_version': 1, 'language': 'en',
            'bible_version': 'RVR1960', 'version': 1, 'estimated_reading_minutes': 5,
            'meta': {
                'character': 'Peter', 'testament': 'new', 'scripture_reference': 'Juan 21:1-19',
                'mood_primary': 'hope', 'accent_color': '#000', 'emoji': '🐟', 'tags': ['faith'],
            },
            'key_verse': {'reference': 'Juan 21:1', 'text': 'text', 'bible_version': 'RVR1960'},
            'cards': [
                {'order': 1, 'type': 'discovery_activation',
                 'discovery_questions': [{'category': 'c', 'question': 'q'}],
                 'prayer': {'title': 't', 'content': 'c'}},
                {'order': 2, 'type': 'completion',
                 'completion_verse': {'reference': 'r', 'text': 't', 'bible_version': 'RVR1960'}},
            ],
        }

    def test_valid_encounter_no_errors(self):
        report = Reporter()
        encounters_schema_checks.check_required_fields('en', self._valid_encounter(), report)
        self.assertEqual(report.errors, 0)
