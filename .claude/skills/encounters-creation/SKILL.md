...
Before drafting `discovery_questions` or `prayer.content`, identify what the reader should take away from the *entire* encounter — not a surface topic like "notice God's presence," but the sharpest, most specific insight the whole arc builds toward.

**Required first step — feeling-mapping pass:** walk every narrative/character/theological card in the encounter and build a table of `feeling felt at this beat → the exact line in that card's content/narrative/revelation_key that transmits it`. The feeling is emotional, not theological (shame, urgency, relief, fear, surprise — not "grace" or "faith" in the abstract), and the paired line must already exist in the text, not be paraphrased into something new. Present this table before selecting anything. From it, choose ~3 feelings with an emotional arc — entry wound, grace/turning point, forward-looking resolution — to become the categories and questions. This prevents the two failure modes seen in practice: inventing dramatic details not in the text (chasing "flow" without an anchor), and picking a thin or generic beat just because it fits a virtue-word category. Category labels should name the feeling/symbol itself when the encounter has a strong enough unique image ("Tu fuego", "Amor en acción") rather than a generic virtue word reused across encounters.
...
- Prefer direct second-person confrontation ("Así como [personaje], ¿tú...?") over hedged hypotheticals ("¿Qué cambiaría si...?", "¿Hay algo así en tu vida...?"). State the grounded fact as confident narration, the same voice the encounter's own cards use — don't lean on quoting the card as a citation crutch to feel anchored. The premise is that every born-again reader already carries some version of the character's shame or unworthiness, regardless of how different their circumstances are — the question should assume this, not merely suggest it.
...
**Concrete case (2026-07-26, Peter's Restoration):** first drafts of a question about the coal-fire memory (Juan 21:9, echoing the denial fire of Juan 18:18) went wrong twice by skipping the feeling-mapping table and polishing sentences directly — first inventing an unsupported sensory detail ("el olor"), then overcorrecting into invented drama ("el fuego vuelve a arder bajo sus pies"). The fix wasn't another wording pass; it was mapping the encounter's cards to feelings first (shame/memory at the coal-fire card, inadequacy-met-with-grace at the ἀγαπᾷς/φιλῶ card, disqualification-overridden-by-commission at the shepherding-task card), which produced grounded, flowing, directly-confrontational questions on the first pass.
