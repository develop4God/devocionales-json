...
> **Current status (2026-04-21):** 40 files · 14 600 entries · 10 languages · 20 Bible versions · ✅ all validation checks pass · ready to merge.
...
- [TL ASND 2026 Quality Gate](#tl-asnd-2026-quality-gate)
...
| `Devocional_year_2025_ar_NAV.json` | ar | NAV | 2025 | 365 |
| `Devocional_year_2026_ar_NAV.json` | ar | NAV | 2026 | 365 |
| `Devocional_year_2025_ar_SVDA.json` | ar | SVDA | 2025 | 365 |
| `Devocional_year_2026_ar_SVDA.json` | ar | SVDA | 2026 | 365 |
| `Devocional_year_2025_tl_ADB.json` | tl | ADB | 2025 | 365 |
| `Devocional_year_2026_tl_ADB.json` | tl | ADB | 2026 | 365 |
| `Devocional_year_2025_tl_ASND.json` | tl | ASND | 2025 | 365 |
| `Devocional_year_2026_tl_ASND.json` | tl | ASND | 2026 | 365 ⚠️ |
...
> ⚠️ `Devocional_year_2026_tl_ASND.json` is currently under AI quality review — see [TL ASND 2026 Quality Gate](#tl-asnd-2026-quality-gate) below.
...
```
devocionales-json/
├── Devocional_year_20XX*.json      ← 40 production files
├── index.json                      ← manifest used by the app API
├── README.md                       ← this file
├── archive/                        ← historical generation artifacts (not deployed)
│   ├── critic_audit_tl_ASND_2026.jsonl  ← deduplicated AI quality-gate audit (365 records)
│   ├── gold_tl_ASND_2026.json           ← OK entries from tl/ASND 2026 audit
│   ├── flagged_tl_ASND_2026.json        ← PAUSE entries from tl/ASND 2026 audit
│   └── errors_tl_ASND_2026.json         ← parse-error entries from tl/ASND 2026 audit
...
│   ├── export_gold.py              ← export OK entries from a critic audit JSONL
│   ├── export_errors.py            ← export parse-error entries from a critic audit JSONL
│   ├── export_flagged.py           ← export PAUSE entries from a critic audit JSONL
...
Checks that every `id` is globally unique across **all 40 files** (14 600 entries).
...
### `export_gold.py` · `export_errors.py` · `export_flagged.py` — critic-audit export tools

These three scripts read a deduplicated critic-audit JSONL (e.g. `archive/critic_audit_tl_ASND_2026.jsonl`) and cross-reference it against the source devotional JSON to produce subsets for downstream processing.

| Script | Output | Purpose |
|---|---|---|
| `export_gold.py` | `archive/gold_tl_ASND_2026.json` | Entries that passed review (action=`reviewed`, verdict=`OK`) |
| `export_errors.py` | `archive/errors_tl_ASND_2026.json` | Entries where the auditor hit token limit or looped before producing JSON (action=`error_parse`) — re-run with `max_tokens=16384` |
| `export_flagged.py` | `archive/flagged_tl_ASND_2026.json` | Entries flagged for manual review (action=`flagged`, verdict=`PAUSE`) — annotated with audit category, confidence, and quoted problematic passage |

```bash
# Run with defaults (targets 2026 tl/ASND)
...
# Filter flagged output by category
python3 devocionales_scripts/export_flagged.py --category register_drift
python3 devocionales_scripts/export_flagged.py --category generic_reflection
python3 devocionales_scripts/export_flagged.py --category prayer_drift

# Target a different audit / source file
python3 devocionales_scripts/export_flagged.py \
    --audit  archive/critic_audit_tl_ASND_2026.jsonl \
    --source Devocional_year_2026_tl_ASND.json \
    --out    /tmp/flagged_review.json
```

---

### Running validation on all 40 files

```bash
cd /path/to/devocionales-json

# Quick pass/fail summary for all 40 files
...
fr_LSG1910,fr_TOB,hi_HERV,hi_HIOV,pt_ARC,pt_NVI,ar_NAV,ar_SVDA,tl_ADB,tl_ASND}.json \
...
```bash
# 1. Content validation — all 40 files
#    (run the loop above; every file must show ✅ PASSED)

# 2. Cross-file duplicate ID check
python3 devocionales_scripts/validate_duplicates.py

# 3. index.json consistency — verify all 20 lang/version combos are present
python3 -c "
...
**Expected results:**
- 40/40 files ✅ PASSED
- 0 duplicate IDs across 14 600 entries
- 20 lang/version combos in `index.json`

**Last validated:** 2026-04-21 — all checks passed.
...
| `ar` | Arabic | NAV · SVDA |
| `tl` | Tagalog (Filipino) | ADB · ASND |

> **HIOV note:** The Hindi OV displays its label in the `versiculo` field as `(ओ.वी.)`, not `HIOV` — this is correct and expected.

---

## TL ASND 2026 Quality Gate

> **Audit run:** 2026-04-20 · **File:** `Devocional_year_2026_tl_ASND.json` · **Entries audited:** 365  
> **Audit JSONL:** `archive/critic_audit_tl_ASND_2026.jsonl` (deduplicated — 365 records, one per entry)

### Summary

| Status | Count | Description |
|---|---|---|
| ✅ OK (`reviewed`) | 278 | Passed two-phase AI review |
| ⚠️ PAUSE (`flagged`) | 58 | Flagged for manual review |
| 🔴 Error (`error_parse`) | 29 | Auditor hit token limit or looped — needs re-run |

### Flagged entry breakdown

| Category | Count | Root cause |
|---|---|---|
| `register_drift` | 32 | **Sub-pattern A (18):** Prayer shifts between *Inyong/Iyong* (formal) and *Mo/mo* (intimate) within the same prayer — systemic generator bug. **Sub-pattern B (14):** Reflections contain Greek word studies (*elegcho, eirene, diakonos, Logos*) or terms like *teolohikal na katotohanan, intelektwal na kaalaman* — breaks devotional tone. |
| `generic_reflection` | 14 | Reflections drift into universal statements not anchored to the specific verse. |
| `prayer_drift` | 11 | Recurring "instrument for others" pivot pattern not present in verse or reflection. |
| `other` | 1 | Miscellaneous quality issue. |

### Key flagged entries (highest priority)

**Register drift — pronoun inconsistency:**
`1Corinto86ASND20270402` · `Juan1412ASND20270303` · `1Juan54ASND20270324` · `Mateo544ASND20270724`

**Register drift — academic/Greek language:**
`Filipos47ASND20260902` · `Juan167-8ASND20260831` · `Juan114ASND20270127`

**Generic reflection:**
`Filipos214ASND20260903` · `Roma328ASND20270517` · `Mateo1926ASND20270426`
...
- [ ] Use `export_flagged.py --category register_drift` to work through pronoun and Greek-language entries
