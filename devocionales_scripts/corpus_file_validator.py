...
class CorpusFileValidator:
    ...
    def validate(
        self, combo: CorpusCombo, data: dict, report: ReportLike
    ) -> "tuple[list, set]":
    ...
    def _check_declared_consistency(
        self, combo: CorpusCombo, entries: list, report: ReportLike
    ) -> None:
        ...
        wrong_version = [
            e.get("id", "?") for e in entries if e.get("version") != combo.version
        ]
        wrong_lang = [
            e.get("id", "?") for e in entries if e.get("language") != combo.lang
        ]
