...
# verify_image_urls is a same-directory sibling script, not an installed
# package — still needs sys.path for that one, unlike shared_validation.
...
from shared_validation.checks.family_check import (
...
def validate_greek_family_patterns(report: Report) -> None:
    ...
    total_errors, total_warnings = run_lexicon_family_check("encounters", lexicon)
    if total_errors:
        report.E(
